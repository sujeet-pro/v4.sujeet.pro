...
# Search Algorithms - Deep Dive into Theory and Practice
...
3. [Binary Search and Variations](#binary-search-and-variations)
4. [Graph Search Algorithms](#graph-search-algorithms)
5. [Specialized Search Algorithms](#specialized-search-algorithms)
6. [Search Algorithm Comparison](#search-algorithm-comparison)
7. [Practical Usage Guide](#practical-usage-guide-when-to-use-each-algorithm)
...
function findUserById(users: User[], id: number): User | undefined {
  ...
  const idx = binarySearch(users.map(u => u.id), id)
...
function findUsersInAgeRange(users: User[], minAge: number, maxAge: number): User[] {
  const startIdx = lowerBound(users.map(u => u.age), minAge)
  const endIdx = upperBound(users.map(u => u.age), maxAge)
...
function findPathForUnit(
  unit: Unit,
  target: Point,
  gameMap: Grid
): Point[] {
  const heuristic = (node: Point) =>
...
function solvePuzzle(initial: PuzzleState, goal: PuzzleState): Move[] {
  const heuristic = (state: PuzzleState) =>
    manhattanDistanceHeuristic(state, goal)
