...
flowchart LR
    subgraph "Protocol Evolution"
        ...
        H2 --> H3["HTTP/3<br/>QUIC + TLS 1.3<br/>1 RTT / 0 RTT"]
    end

    subgraph "Key Improvements"
        HOL["HOL Blocking"] --> MUX["Multiplexing"]
        MUX --> STREAM["Stream Independence"]
    end
```

<figcaption>Evolution from HTTP/1.1 to HTTP/3 showing handshake latency reduction and head-of-line blocking solutions</figcaption>

</figure>

## TLDR

**HTTP protocol evolution** represents a systematic approach to solving performance bottlenecks at successive layers of the network stack, from application-layer head-of-line blocking in HTTP/1.1 to transport-layer independence in HTTP/3 with QUIC.

### Protocol Versions and Latency

- **HTTP/1.1**: 3 RTT connection (TCP + TLS 1.2), application-layer HOL blocking, 6 parallel connections per hostname
- **HTTP/2**: 2 RTT connection (TCP + TLS 1.3), binary framing with multiplexing, but TCP-level HOL blocking persists
- **HTTP/3**: 1 RTT (0 RTT resumption) via QUIC, stream independence eliminates transport HOL blocking entirely
- **TLS 1.3**: Mandatory forward secrecy, 1 RTT handshake, 0 RTT resumption with PSK (replay risk for non-idempotent requests)

### Protocol Negotiation Mechanisms

- **ALPN (Application-Layer Protocol Negotiation)**: TLS extension for seamless HTTP/2 negotiation without extra RTT
- **Alt-Svc Header**: Server-initiated HTTP/3 upgrade advertisement for QUIC connection attempts
- **DNS SVCB/HTTPS Records**: Pre-connection protocol discovery with IP hints and ECH config
- **Fallback Strategy**: Browsers always maintain TCP fallback when QUIC fails or is blocked

### QUIC Architecture Advantages

- **User-Space Transport**: Protocol evolution independent of OS kernel updates
- **Connection Migration**: Seamless handoff between networks (WiFi → cellular) without re-handshake
- **Pluggable Congestion Control**: CUBIC, BBR algorithms deployable via application updates
- **Integrated TLS 1.3**: Cryptographic handshake combined with transport setup

### Implementation Considerations

- **Server Support**: Nginx 1.25+, Caddy (default), CDN-based adoption simplest path
- **DNS Configuration**: Publish SVCB/HTTPS records with ALPN and IP hints for optimal discovery
- **0-RTT Security**: Restrict to idempotent methods (GET, HEAD, OPTIONS) due to replay attack risk
- **Monitoring**: Track protocol adoption rates, handshake times, and fallback frequency
...
## 7. Protocol Negotiation and Upgrade Mechanisms
...
## 8. Browser Protocol Negotiation Mechanisms
...
## 9. Performance Characteristics and Decision Factors
...
## 10. Security Implications and Network Visibility
...
## 11. Strategic Implementation Considerations
