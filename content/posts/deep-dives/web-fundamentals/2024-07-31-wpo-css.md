...
flowchart LR
    ...
    subgraph "Optimizations"
        CRITICAL["Critical CSS<br/>Inline ATF"]
        CONTAIN["Containment<br/>Isolate reflows"]
        COMPOSITE["Composite-only<br/>transform/opacity"]
    end
```

<figcaption>CSS performance optimization stages: delivery, parsing, rendering, and animation</figcaption>

</figure>

## TLDR

**CSS performance optimization** targets four stages: delivery (reduce blocking time), parsing (minimize CSSOM size), rendering (isolate layout work), and animation (use compositor-only properties).
...
- **Specificity cost**: Browsers match selectors right-to-left; highly specific selectors aren't slow, but overly complex ones are
- **Avoid universal selectors**: `* { }` forces style recalculation on all elements
- **Keep selectors shallow**: Prefer `.card-title` over `.card > .content > .header > h2`
- **Modern CSS impact**: `:has()`, `:is()`, `:where()` can affect selector matching performance

### Layout Containment

- **`contain: layout`**: Isolates element's layout from rest of document
- **`contain: paint`**: Element's contents don't render outside its bounds
- **`contain: size`**: Element's size doesn't depend on children (must specify explicit size)
- **`content-visibility: auto`**: Skip rendering for off-screen content (huge paint savings)

### Animation Performance

- **Composite-only properties**: `transform` and `opacity` avoid layout/paint (GPU-accelerated)
- **`will-change`**: Hints browser to create compositor layer ahead of animation
- **Avoid**: Animating `width`, `height`, `top`, `left`, `margin` (trigger layout)
- **Forced synchronous layout**: Reading layout properties during animation causes jank

### Paint Optimization

- **Complexity cost**: `box-shadow`, `blur`, `filter` are expensive; test with "Paint Flashing" in DevTools
- **Layer promotion**: `will-change: transform` or `transform: translateZ(0)` creates compositor layer
- **Trade-off**: Too many layers increase memory usage and compositing overhead
- **Target**: Keep paint under 2ms per frame for 60fps
...
- [CSS Houdini](https://developer.mozilla.org/en-US/docs/Web/API/Houdini_APIs) - Paint and Animation Worklets
