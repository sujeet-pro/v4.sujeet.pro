...
Path B (CRDT) details are covered in [CRDTs for Collaborative Systems](/articles/system-design/core-distributed-patterns/crdt-for-collaborative-systems).
...
```json
{
  ...
  "collaborators": [
    { "id": "user_456", "name": "Bob", "online": true }
  ]
}
```
...
class SnapshotWorker {
  ...
  async processDocument(documentId: string): Promise<void> {
    ...
    if (opsSinceSnapshot < this.SNAPSHOT_THRESHOLD &&
        timeSinceSnapshot < this.SNAPSHOT_INTERVAL_MS) {
    ...
    await this.opLog.setTTL(
      documentId,
      0,
      doc.currentRevision - 100,
      TTL_30_DAYS
    );
  }
}
...
**Related Articles:**
- [Operational Transformation](/articles/system-design/core-distributed-patterns/operational-transformation) - Deep dive into OT algorithms
- [CRDTs for Collaborative Systems](/articles/system-design/core-distributed-patterns/crdt-for-collaborative-systems) - Alternative approach for offline-first
