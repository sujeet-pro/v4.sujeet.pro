...
function applyTenantOverrides(
  ...
  const override = tenantConfig.regulations.overrides?.[`${detected.country}-${detected.region}`]
...
function setupInvalidationListener(tenantId: string): void {
  const eventSource = new EventSource(
    `/api/v1/consent/events?tenant=${tenantId}`
  );
...
async function getBannerVariant(
  ...
  const variant = experiment.variants.find(v =>
    bucket >= v.startBucket && bucket < v.endBucket
  );
...
- [LGPD Text](https://www.gov.br/cidadania/pt-br/acesso-a-informacao/lgpd) - Brazilian privacy law
