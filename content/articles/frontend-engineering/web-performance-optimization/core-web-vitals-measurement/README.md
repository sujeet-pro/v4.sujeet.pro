...
onLCP((metric) => {
  ...
  const bottleneck = subparts.reduce((max, curr) =>
...
onINP((metric) => {
  ...
  const dominant = phases.reduce((max, curr) =>
    curr.value > max.value ? curr : max
  );
...
function getRecommendation(phase: string): string {
  switch (phase) {
    case 'inputDelay': return 'Break up long tasks with scheduler.yield()';
    case 'processingTime': return 'Optimize event handler or defer work';
    case 'presentationDelay': return 'Reduce DOM mutations, avoid forced reflows';
    default: return '';
  }
}
```
