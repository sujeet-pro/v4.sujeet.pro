...
export async function getAllArticlesDetailed(): Promise<ArticleDetailedInfo[]> {
  ...
  const articleById = new Map(articles.map((article) => [article.postId, article]))
  ...
  // Include draft articles filtered from browseContent in production builds.
  // Draft pages are generated by getStaticPaths and should render (with noIndex).
  for (const article of articles) {
    if (processedIds.has(article.postId) || !article.isDraft) continue

    const card: ArticleCardInfo = {
      id: article.postId,
      title: article.title,
      description: article.description,
      href: article.href,
    ...
    detailedArticles.push({
      ...
      card,
      topic,
      toc: headingsById[article.postId] ?? [],
      prev: null,
      next: null,
    })
  }
