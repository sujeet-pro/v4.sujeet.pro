...
- [ ] Article is in `content/posts.jsonc` order array
- [ ] Article is in topic's `meta.jsonc` order array (`content/articles/<category>/<topic>/meta.jsonc`)
- [ ] If cornerstone content, consider if it should be in `home.jsonc` featured articles
...
#### Adding to Configuration Files

If article is missing from configuration files, add it:

```jsonc
// content/articles/<category>/<topic>/meta.jsonc
{ "order": ["existing-article", "this-article"] }

// content/posts.jsonc
{
  "order": [
    // ... existing
    "category/topic/this-article",
  ],
}
```

Run `npm run validate:content` to verify all configuration is correct.
